...
# Choose the parameter that will be predicted and create a new dataframe with it in the first column and the resuming stats
target_param_N <- "pop_size"
learning_data_N <- bind_cols(y = params_N[[target_param_N]], stats_table_N) %>% rename(!!target_param_N := y)
...
```{r = ABCRF model - Reailized_Ne - specific data}

# Choose the parameter that will be predicted and create a new dataframe with it in the first column and the resuming stats
target_param_Ne <- "Harmonic_Ne"
learning_data_Ne <- bind_cols(y = params_Ne[[target_param_Ne]], stats_table_Ne) %>% rename(!!target_param_Ne := y)
