...
def parse_ld(content, pop):
    ...
    if block:
        ...
        results[f"LD_Ne_Pop{pop}"] = [results.get(f"LD_Ne_{th}_Pop{pop}") for th in ["0.05", "0.02", "0.01", "0.00"]]
        results[f"LD_r2_Pop{pop}"] = [results.get(f"r2_overall_{th}_Pop{pop}") for th in ["0.05", "0.02", "0.01", "0.00"]]
    return results
...
def parse_he(content, pop):
    ...
    if block:
        ...
        results[f"HE_Neb_mean_Pop{pop}"] = [results.get(f"HE_Neb_mean_{th}_Pop{pop}") for th in ["0.05", "0.02", "0.01", "0.00"]]
        results[f"HE_weighted_D_mean_Pop{pop}"] = [results.get(f"HE_weighted_D_mean_{th}_Pop{pop}") for th in ["0.05", "0.02", "0.01", "0.00"]]
    return results
